(function ($) {
    ...
    function TreeMap($div, options) {
        ...
        this.centerLabelVertically = false;
    ...
    TreeMap.prototype.paint = function (nodeList) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
            ...
            var $box = $('<article id=' + boxId + '></article>');
            ...
            $box.addClass('evt').attr({
                'data-func' : 'clickBoxTreemap',
                'data-deploy' : 'false',
                'data-proyect' : node.proyectID,
                'data-url' : node.url,
                'data-tendencia' : node.tendencia
            });
            
            var $content = $('<a href="'+ node.url +'" title="Ver detalle del proyecto ' + node.label + '" rel="nofollow">' + node.label + '</a>');
            $content.on('click.bigblock', function( event ){
...
//                $content.remove();
    ...
    TreeMap.prototype.splitFairly = function (nodeList) {
        ...
        for (var midPoint = 0; midPoint < length; midPoint++) {
            if (midPoint > 0 && (accValue + nodeList[midPoint].montoTotal > halfValue))
                break;
            accValue += nodeList[midPoint].montoTotal;
        }
    ...
    $.fn.treemap = function (json, options) {
        ...
        return this.fadeOut('slow', function () {
            self.empty().fadeIn('slow', function () {
